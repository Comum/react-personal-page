...
const getTechList = (techs = []) => {
	...
	return techs.map(tech => <TechElement key={tech.id}>{tech.name}</TechElement>);
};
...
const WorksTechs = ({ techs }) => {
	...
	return (
		<SectionContainer title={title} type={type} colour={colour} iconPath={iconPath} bgColour={bgColour}>
			<TechContainer>{techList}</TechContainer>
		</SectionContainer>
	);
};

WorksTechs.propTypes = {
	techs: PropTypes.oneOfType([PropTypes.array, PropTypes.object]),
};
